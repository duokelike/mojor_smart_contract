...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    string public constant name = "Mojor Exchange Contract";
    ...
    bytes32 private constant _NAME_HASH = 0x0876e5223d45bbb8f6c499ef20ce8c16e206191f25c4f136073a0b8f306d25fe;
    ...
    bytes32 public constant DOMAIN_SEPARATOR = 0x9902efea5110bd2d765923ab5863f661d55a472b88ef149e5d1669329e32aec8;
...
_NAME_HASH, // keccak256("Mojor Exchange Contract")
...
/**
* @dev Change the protocol fee  (owner only)
     * @param newMojorProtocolFee New protocol fee percent
     */
function changeMojorProtocolFee(uint newMojorProtocolFee)
public
onlyOwner
{
mojorProtocolFee = newMojorProtocolFee;
}
...
sell.staticTarget.transfer(makerProtocolFee);
} else {
transferTokens(sell.paymentToken, sell.maker, sell.staticTarget, makerProtocolFee);
}
}
...
sell.staticTarget.transfer(takerProtocolFee);
} else {
transferTokens(sell.paymentToken, buy.maker, sell.staticTarget, takerProtocolFee);
}
}

/*about mojor protocol fee */
if (mojorProtocolFee > 0){
uint mojorProtocolFeeCalculate = SafeMath.div(SafeMath.mul(mojorProtocolFee, price), INVERSE_BASIS_POINT);
if (sell.paymentToken == address(0)) {
requiredAmount = SafeMath.add(requiredAmount, mojorProtocolFeeCalculate);
protocolFeeRecipient.transfer(mojorProtocolFeeCalculate);
} else {
transferTokens(sell.paymentToken, buy.maker, protocolFeeRecipient, mojorProtocolFeeCalculate);
}
}
...
transferTokens(sell.paymentToken, buy.maker, buy.staticTarget, makerProtocolFee);
}

if (buy.takerProtocolFee > 0) {
takerProtocolFee = SafeMath.div(SafeMath.mul(buy.takerProtocolFee, price), INVERSE_BASIS_POINT);
transferTokens(sell.paymentToken, sell.maker, buy.staticTarget, takerProtocolFee);
}

/*about mojor protocol fee(half point)*/
if (mojorProtocolFee > 0){
uint mojorProtocolBuyCalculate = SafeMath.div(SafeMath.mul(mojorProtocolFee, price), INVERSE_BASIS_POINT * 2);
transferTokens(sell.paymentToken, buy.maker, protocolFeeRecipient, mojorProtocolFeeCalculate);
transferTokens(sell.paymentToken, sell.maker, protocolFeeRecipient, mojorProtocolFeeCalculate);
}
...
contract MojorExchangeWithBulkCancellations is Exchange {
string public constant codename = "Mojor Bulk Smash";

/**
 * @dev Initialize a MojorExchange instance
...
/* Delay period for adding an authenticated contract.
   This mitigates a particular class of potential attack on the Mojor DAO (which owns this registry) - if at any point the value of assets held by proxy contracts exceeded the value of half the WYV supply (votes in the DAO),
   a malicious but rational attacker could buy half the Mojor and grant themselves access to all the proxy contracts. A delay period renders this attack nonthreatening - given two weeks, if that happened, users would have
